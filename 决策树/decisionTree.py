...
def calcShannonEnt(dataSet):
    ...
    for featVec in dataSet:
        ...
        if currentLabel not in labelCounts.keys(): #keys()返回所有的键
            labelCounts[currentLabel] = 0
        labelCounts[currentLabel] += 1 # 计算每一类各多少个， {'Y': 4, 'N': 3}
...
def chooseBestFeatureToSplit(dataSet):
    ...
    for i in range(numFeatures):
        ...
        uniqueVals = set(featList)                              #每个特征的可能取值
        ...
        for value in uniqueVals:
            ...
            splitInfo += -prob * log(prob,2)                    #C4.5求信息增益率

        infoGain = baseEntropy - newEntropy                     #这个特征的信息增益
...
def createTree(dataSet, labels):
    ...
    if classList.count(classList[0]) == len(classList):         # classList所有元素都相等，即类别完全相同，停止划分
    ...
    bestFeatLabel = labels[bestFeat]                            # 选择最大的gain ratio对应的feature   #outlook -> windy
